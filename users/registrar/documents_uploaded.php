    ...
    <div class="d-flex">
        ...
        <div class="sidebar" style="width: 250px; position: fixed; top: 0; left: 0;">
            ...
            <nav class="nav flex-column">
                ...
                <a href="documents_uploaded.php" class="nav-link active">
    ...
    <script>
        ...
        function confirmDelete(fileGroup) {
            Swal.fire({
                ...
                text: `Are you sure you want to delete all records for "${fileGroup}"?`,
                icon: 'warning',
                input: 'password',
                inputPlaceholder: 'Enter password to confirm deletion',
                inputAttributes: {
                    autocapitalize: 'off',
                    autocorrect: 'off'
                },
                ...
                inputValidator: (value) => {
                    if (!value) {
                        return 'Password is required to delete records!'
                    }
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    fetch(`delete_document_records.php?file_group=${encodeURIComponent(fileGroup)}&campus=<?php echo $sheet_name; ?>&password=${encodeURIComponent(password)}`)
        ...
        function confirmDeleteCORCOG(category) {
            Swal.fire({
                title: 'Confirm Deletion',
                text: `Are you sure you want to delete all records for "${category}"?`,
                icon: 'warning',
                input: 'password',
                inputPlaceholder: 'Enter password to confirm deletion',
                inputAttributes: {
                ...
                if (result.isConfirmed) {
                    ...
                    // Make delete request with password
                    fetch(`delete_document_records.php?category=${encodeURIComponent(category)}&campus=<?php echo $sheet_name; ?>&password=${encodeURIComponent(password)}`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                Swal.fire({
