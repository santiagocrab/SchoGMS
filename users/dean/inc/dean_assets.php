<?php
/**
 * Lean CSS/JS for college dean pages.
 */
require_once __DIR__ . '/../../../inc/schogms_app_scripts.php';

if (!function_exists('schogms_dean_head')) {
    function schogms_dean_head(bool $datatables = false): void
    {
...
if (!function_exists('schogms_dean_footer_scripts')) {
    /** @param array{datatables?:bool,sweetalert?:bool} $opts */
    function schogms_dean_footer_scripts(array $opts = []): void
    {
        ...
        if (!empty($opts['sweetalert']) && !$sweetalertDone) {
            $sweetalertDone = true;
        ...
        if (!empty($opts['datatables']) && !$datatablesDone) {
            ...
            echo '<script>jQuery(function(){if(!jQuery.fn.DataTable)return;var o={pageLength:25,order:[]};jQuery("#zero_config").each(function(){if(!jQuery.fn.dataTable.isDataTable(this))jQuery(this).DataTable(o);});if(typeof window.schogmsInitTopbarUi==="function")window.schogmsInitTopbarUi();});</script>' . "\n";
        }
    }
}
