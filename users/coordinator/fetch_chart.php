...
try {
    ...
    if (!empty($campus)) {
        $totalRecordsQuery = "SELECT COUNT(*) AS total_records FROM ched_masterlist WHERE sheet_name = '$campus'";
    ...
    $totalCourses = [];
    if (!empty($campus)) {
        $stmt = $conn->prepare("SELECT course_program_enrolled, COUNT(*) AS count FROM ched_masterlist WHERE sheet_name = ? GROUP BY course_program_enrolled ORDER BY count DESC LIMIT 10");
        if ($stmt) {
    ...
    $totalFileGroups = [];
    if (!empty($campus)) {
        $fileGroupsQuery = "SELECT file_group, COUNT(*) AS count FROM ched_masterlist WHERE sheet_name = '$campus' GROUP BY file_group ORDER BY count DESC LIMIT 10";
    ...
    error_log("Chart fetch error: " . $e->getMessage());
}
