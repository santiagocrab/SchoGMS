...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    INSERT INTO ched_masterlist (
        seq, app_no, award_no, lastname, firstname, extname, middlename, 
...
";
        ...
        foreach ($dataRows as $row) {
            if (array_filter($row)) { // Skip empty rows
                ...
                $stmt->bind_param(
                    "ssssssssssssssss",  // Corresponds to 19 placeholders
                    $row['A'], // SEQ
                    $row['B'], // APP NO
                    $row['C'], // AWARD NO.
                    $row['D'], // LASTNAME
        ...
        echo json_encode(['success' => true, 'message' => 'Data successfully uploaded to ched_masterlist.']);
    } catch (Exception $e) {
