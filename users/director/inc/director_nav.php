<?php
/**
 * Unified campus director top bar + sidebar.
 */

if (!function_exists('schogms_director_nav_script')) {
    function schogms_director_nav_script(): string
    {
        return basename($_SERVER['SCRIPT_NAME'] ?? '');
    }
}

if (!function_exists('schogms_director_nav_active')) {
    function schogms_director_nav_active(string $script, array $aliases = []): bool
    {
        $current = schogms_director_nav_script();

        return $current === $script || in_array($current, $aliases, true);
    }
}

if (!function_exists('schogms_director_nav_link_class')) {
    function schogms_director_nav_link_class(string $script, array $aliases = []): string
    {
        $class = 'sidebar-link sidebar-link';

        return schogms_director_nav_active($script, $aliases)
            ? $class . ' active'
            : $class;
    }
}

if (!function_exists('schogms_director_render_topbar')) {
    function schogms_director_render_topbar(?string $pageTitle = null): void
    {
        global $fullname;
        $title = $pageTitle ?? 'Scholarship and Grants Management System';
        $user = htmlspecialchars((string) ($fullname ?? 'Director'), ENT_QUOTES, 'UTF-8');
        ?>
...
if (!function_exists('schogms_director_sidebar_menu_items')) {
    function schogms_director_sidebar_menu_items(): void
    {
        ?>
                        <li class="sidebar-item">
                            <a class="<?= schogms_director_nav_link_class('index.php') ?>" href="index.php">
...
if (!function_exists('schogms_director_render_sidebar')) {
    function schogms_director_render_sidebar(): void
    {
        ?>
        <aside class="left-sidebar" data-sidebarbg="skin6">
            <div class="scroll-sidebar" data-sidebarbg="skin6">
                <nav class="sidebar-nav">
                    <ul id="sidebarnav">
                        <?php schogms_director_sidebar_menu_items(); ?>
                    </ul>
                </nav>
            </div>
        </aside>
...
if (!function_exists('schogms_director_shell_open')) {
    function schogms_director_shell_open(?string $pageTitle = null): void
    {
        ...
        <?php
        schogms_director_render_topbar($pageTitle);
        schogms_director_render_sidebar();
        ?>
