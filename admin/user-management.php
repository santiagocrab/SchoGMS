...
<body>
    ...
    <div id="main-wrapper" data-theme="light" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
        ...
        <div class="page-wrapper">
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    ...
                    <div class="col-3 align-self-center">
                        <div class="customize-input float-right">
                            <button type="button" class="btn waves-effect waves-light btn-rounded btn-success"
                                data-toggle="modal" data-target="#campusCreateModal">
                                Manage Campus
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="submit_user.php" method="post">
                                ...
                                // Fetch campuses from the `campuses` table
                                $query = "SELECT campus_name FROM campuses ORDER BY campus_name ASC";
                                $result = mysqli_query($conn, $query);
                                ?>
                                <div class="form-group">
                                    <label for="userCampus">List of SKSU Campus</label>
                                    <select class="form-control" id="userCampus" name="userCampus">
                                        <option value="" disabled selected>Select</option>
                                        <?php
                                        if ($result) {
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                echo '<option value="' . htmlspecialchars($row['campus_name']) . '" readonly>' . htmlspecialchars($row['campus_name']) . '</option>';
                                            }
                                        } else {
                                            echo '<option value="" disabled>Error loading campuses</option>';
                                        }
                                        ?>
                                        <option value="Chairman">Chairman (No Campus)</option>
                                    </select>
                                </div>
                                <!-- Submit Button -->
                                <div class="form-group">
            ...
            <div class="modal fade" id="campusCreateModal" tabindex="-1" role="dialog"
                aria-labelledby="campusCreateModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="campusCreateModalLabel">Manage SKSU Campus</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            ...
                            // Fetch campuses from the `campuses` table
                            $query = "SELECT id, campus_name FROM campuses ORDER BY campus_name ASC";
                            $result = mysqli_query($conn, $query);
                            ?>
                            ...
                            <form id="campusForm">
                                ...
                                <div class="form-group">
                                    <label for="deluserCampus">List of SKSU Campuses</label>
                                    <select class="form-control" id="deluserCampus" name="deluserCampus">
                                ...
                                <div class="form-group">
                                    ...
                                    <button type="button" class="btn btn-danger" id="deleteCampus">Delete Selected
                                        Campus</button>
                                </div>
                            </form>
                                ...
                                <div class="table-responsive">
                                    <table id="zero_config" class="table table-striped table-bordered no-wrap">
                                        ...
                                        <tbody>
                                            ...
                                            $sql = "SELECT user_id, name, email, created_at, role, campus, status FROM users";
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="updateUserForm">
                                    ...
                                    <div class="form-group">
                                        <label for="userCampus">List of SKSU Campus</label>
                                        <select class="form-control" id="newUserCampus" name="newUserCampus" required>
                                            <option value="" disabled selected>Select</option>
                                            <?php
                                            if ($result) {
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    echo '<option value="' . htmlspecialchars($row['campus_name']) . '">' . htmlspecialchars($row['campus_name']) . '</option>';
                                                }
                                            } else {
                                                echo '<option value="" disabled>Error loading campuses</option>';
                                            }
            ...
            $.ajax({
                ...
                success: function (userData) {
                    ...
                    if (userData.success) {
                        ...
                        $('#newUserCampus').val(userData.campus);  // Fill username field
    ...
    <script>
        document.querySelector('#campusForm').addEventListener('submit', async (event) => {
            event.preventDefault(); // Prevent default form submission

            const formData = new FormData(event.target);

            try {
                const response = await fetch('submit_campus.php', {
                    method: 'POST',
                    body: formData,
                });
                ...
                if (result.success) {
                    Swal.fire({
                        ...
                        $('#campusCreateModal').modal('hide'); // Close modal
                        setTimeout(() => {
    ...
    <script>
        document.getElementById('deleteCampus').addEventListener('click', async () => {
            const selectedCampus = document.getElementById('deluserCampus').value; // Get selected campus ID

            if (!selectedCampus) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Warning',
                    text: 'Please select a campus to delete.',
                });
                return;
            }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await fetch('delete_campus.php', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                            body: `campusId=${selectedCampus}`
                        });
